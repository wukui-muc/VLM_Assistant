...
import sys
from networks import Critic, Actor,CNN_LSTM_GoalCondition_bbox
import numpy as np
import math
import copy
import time
...
class CQLSAC_CNN_LSTM_GoalCondition_bbox(nn.Module):
    ...
    def __init__(self,
        ...
        """
        super(CQLSAC_CNN_LSTM_GoalCondition_bbox, self).__init__()
        ...
        self.CNN_LSTM = CNN_LSTM_GoalCondition_bbox(state_size=self.state_size,
    ...
    def normalized_action(self,actions):
    ...
    def normalized_goal(self,goal):
        goal = np.array(goal.cpu())
        # action space 归一化
        # 定义每个维度的最小和最大值
        min_val = np.array([-30]).astype(np.float32)
        max_val = np.array([30]).astype(np.float32)

        # 将数据归一化到0到1的范围
        normalized_data = ((goal - min_val) / (max_val - min_val)).astype(np.float32)

        # 将数据归一化到-1到1的范围
        normalized_data = (2 * normalized_data - 1).astype(np.float32)
        goal = torch.from_numpy(normalized_data).to(self.device)
        return goal
    ...
    def learn(self, experiences):
        ...
        """
        states, actions, rewards, next_states, dones,goal,next_goal = experiences

        batch_size=states.shape[0]
        actions = self.normalized_action(actions)
        # goal = torch.tensor((goal_dire,goal_distance)).to(self.device)
        #--------------------------------Image processing------------------------#
        # exp_distance_norm = ((self.exp_distance-100)/400) #对期望距离输入归一化 （100-500）区间
        states ,reward_head= self.CNN_LSTM(states)
        states=states.reshape(batch_size*self.lstm_seq_len,-1)
        reward_loss = F.mse_loss(reward_head.squeeze(dim = -1), rewards)

        # states = torch.cat((states, torch.ones((batch_size*self.lstm_seq_len, 1)).cuda() * torch.tensor(exp_distance_norm).cuda()),
        #                              dim=1)
        # states = torch.cat(
        #     (states, goal.reshape(batch_size*self.lstm_seq_len,-1)),
        #     dim=1)
        with torch.no_grad():
            next_states, goal_head_next= self.CNN_LSTM(next_states)
            next_states = next_states.reshape(batch_size*self.lstm_seq_len,-1)
            # next_states = torch.cat((next_states, goal.reshape(batch_size*self.lstm_seq_len,-1)), dim=1)
        ...
        total_c1_loss = critic1_loss + cql1_scaled_loss +reward_loss
        total_c2_loss = critic2_loss + cql2_scaled_loss +reward_loss
        ...
        return (q1_train.item(),q2_train.item(),actor_loss.item(), alpha_loss.item(), critic1_loss.item(), critic2_loss.item(), cql1_scaled_loss.item(),cql2_scaled_loss.item(), current_alpha, cql_alpha_loss.item(), cql_alpha.item(),reward_loss.item())
