...
class LSTM_CLIP_GoalCondition_bbox(nn.Module):
    def __init__(self, clip_size, action_size, hidden_size,stack_frames,lstm_out,lstm_layer):
        super(LSTM_CLIP_GoalCondition_bbox, self).__init__()
        self.input_shape = clip_size
        self.action_size = action_size
        self.stack_frames = stack_frames
        # self.bn = torch.nn.BatchNorm2d(self.input_shape[0], eps=1e-05, momentum=0.1,
        #                      affine=True, track_running_stats=True)
        self.fc1 = nn.Linear(self.input_shape, hidden_size)
        self.fc2 = nn.Linear(hidden_size, self.input_shape)
        ...
        self.lstm = nn.LSTM(input_size=self.input_shape, hidden_size=self.lstm_out, num_layers=self.lstm_layer,
                            batch_first=True)
        self.fc_reward_1 = nn.Linear(self.lstm_out, 128)
        self.fc_reward_2 = nn.Linear(128, 1)
    ...
    def forward(self, clip_input):
        ...
        x = self.fc2(self.fc1(clip_input))
        batch_size = clip_input.shape[0]
        seq_len = clip_input.shape[1]
        # input = self.bn(input)
        ...
        reward_head = self.fc_reward_2(self.fc_reward_1(lstm_out))
        # self.ht=ht
        # self.ct=ct
        # x = torch.relu(self.head_1(x))
        # out = torch.relu(self.ff_1(x))

        return lstm_out, reward_head
    ...
    def inference(self, input, ht=None, ct=None):
        ...
        return x, ht, ct
class CNN_LSTM_GoalCondition_bbox(nn.Module):
    def __init__(self, state_size, action_size, hidden_size,stack_frames,lstm_out,lstm_layer):
        super(CNN_LSTM_GoalCondition_bbox, self).__init__()
    ...
    def forward(self, image_input):
        ...
        seq_len = image_input.shape[1]
        input = image_input.reshape(-1, image_input.shape[-3], image_input.shape[-2], image_input.shape[-1])
        # input = self.bn(input)
        x=self.CNN_Simple(input)
        x=x.reshape(x.shape[0],-1)
        x=x.reshape(batch_size,seq_len,-1)
    ...
    def inference(self,input,ht=None,ct=None):
        ...
        return x,ht,ct
class CNN_LSTM_GoalCondition(nn.Module):
    def __init__(self, state_size, action_size, hidden_size,stack_frames,lstm_out,lstm_layer):
        super(CNN_LSTM_GoalCondition, self).__init__()
        self.input_shape = state_size
        self.action_size = action_size
        self.stack_frames = stack_frames
        # self.bn = torch.nn.BatchNorm2d(self.input_shape[0], eps=1e-05, momentum=0.1,
        ...
        # self.outdim = layer_size
        self.outdim=self.lstm_out
        self.lstm = nn.LSTM(input_size=self.cnn_dim+4, hidden_size=self.lstm_out, num_layers=self.lstm_layer,batch_first=True)
        self.fc_reward_1 = nn.Linear(self.lstm_out,128)
        self.fc_reward_2 = nn.Linear(128,1)
        ...
        #
        # self.ff_1 = nn.Linear(layer_size, layer_size)
    def forward(self, image_input,goal_input):
        """
        ...
        x = torch.cat((x,goal_input),dim=-1)
        # if self.ht == None or self.ct == None:
        #     x, (ht, ct) = self.lstm(x)
        # else:
        lstm_out, (ht,ct) = self.lstm(x,(h0,c0))
        # reward_feat=torch.cat((lstm_out,goal_input),dim=-1)
        reward_head= self.fc_reward_2(self.fc_reward_1(lstm_out))
        # self.ht=ht
        # self.ct=ct
        # x = torch.relu(self.head_1(x))
        ...
        return lstm_out,reward_head
    def inference(self,input,goal,ht=None,ct=None):
        ...
        return x,ht,ct
class CNN_LSTM_CLIPTextGoalCondition(nn.Module):
    def __init__(self, state_size, action_size, hidden_size,stack_frames,lstm_out,lstm_layer):
        super(CNN_LSTM_CLIPTextGoalCondition, self).__init__()
        self.input_shape = state_size
        self.action_size = action_size
        self.stack_frames = stack_frames
        # self.bn = torch.nn.BatchNorm2d(self.input_shape[0], eps=1e-05, momentum=0.1,
        ...
        # self.outdim = layer_size
        self.outdim=self.lstm_out
        self.lstm = nn.LSTM(input_size=self.cnn_dim+512, hidden_size=self.lstm_out, num_layers=self.lstm_layer,batch_first=True)
        self.fc_reward_1 = nn.Linear(self.lstm_out,128)
        self.fc_reward_2 = nn.Linear(128,1)
