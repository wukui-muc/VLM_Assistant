...
class ExperienceManager:
    ...
    def call_api(self,recover_state):
       ...
       #  """
        ...
        prompt=f"""
            Objectives:
            You are an intelligent assistant.
            Your task is to help a visual tracking robot identify what caused the robot lose the target person from view, then reasoning a list of actions to help the robot recover from failure.
            Based on the robot's three continuous first-person view observations, which is sampled before failure with a 5-step interval, you need to format a structured failure reason first which could reflect the failure reason, then reasoning a suitable recover action list.
            
            # Input
            1. The robot's three continuous first-person view observations, which is sampled before failure with a 5-step interval.
            
            # Considering strategy
            You need to reasoning from the continous three observations and answer the following three questions:
            **Task 1**: What caused the robot's lost the target person from view, generate a short concise sentence, such as: <Occlusion by target's surrounding environment>, <Target's sudden turn caused out of view>, <Unknown Failure>.  
            **Task 2**: If the target failed because obvious environment structures, provide a short description sentence of the structure or objects, - <description to the structures/objects>, otherwise filled with "None".
            **Task 3**: Based on the three observations, describe the last known target spatial position related to the robot. If the environment structure are applicable, use it as a landmark for description, - <position description along with the landmark>, otherwise, directly describe the target last known position..
            **Task 4**: Generate a list of five continuous actions to help the robot bring back the target into view, such as: [Move Forward, Turn Left, Move Forward, Turn Right, Turn Right]. All available actions are: 
                - Move Forward: Propel the agent forward by 1 meter.
                - Move Backward: Propel the agent backward by 1 meter.
        ...
        context_analysis=answer.lower().split('-')[:-1]
        recovery_action=answer.lower().split('-')[-1]
        answer = f"[Context analysis]: {context_analysis}\n[Recovery action]: {recovery_action}"
        ...
        #     analysis = analysis
        #
        print('recover action before revise:',answer.lower().split('recovery action')[1])
        # try:

        Exp1, Exp2, Exp3 = self.get_exp(str(context_analysis))
        Exp = [
            [Exp1.get("analysis"), Exp1.get("action"), Exp1.get("fail_reason"), Exp1.get("suggestion")],
            [Exp2.get("analysis"), Exp2.get("action"), Exp2.get("fail_reason"), Exp2.get("suggestion")],
            [Exp3.get("analysis"), Exp3.get("action"), Exp3.get("fail_reason"), Exp3.get("suggestion")]
    ...
    def critic(self,image_list, analysis, action):
        ...
        # BACKGROUND
        # Role: Robot Action Verification Expert
        You are responsible for analyzing why the robot did not complete the recover from tracking failure as expected, based on the visual images, action sequences, and context analysis, and providing suggestions for improvement.
        ...
        # THINKING PROCESS
        1. Analyze if the context analysis correctly represent the failure reason and the last known target position.
        2. Analyze the provided image sequence to reflect the robot’s actual behavior.
        3. Compare the actual action sequence to the expected outcome from the context analysis. Identify why the execution led to the observed behavior in the image instead of the expected result.
        4. Based on the identified discrepancies, provide the reasons for the failure and suggest modifications to the action sequence.
        ...
        # OUTPUT
        <DISCREPANCY_ANALYSIS>
            <!-- Concise reasoning sentence, explaning what caused the differences between the expected outcome and the actual movement -->
        </DISCREPANCY_ANALYSIS>

        <ADJUSTMENT_SUGGESTION>
            <!-- Suggested changes to the action sequence, without further explanation -->
            Example: [Turn right, Move Forward, Move Forward, Turn Left, Turn Left].
        </ADJUSTMENT_SUGGESTION>
        """
        ...
        prompt = prompt_reflection_v2.format(analysis=analysis, action=action)
        while True:
            try:
                response = client.chat.completions.create(
